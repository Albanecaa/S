...
def draw(img, props):
    for list in props:
        for pt1, pt2, tpos, char in list:
            ...
            cv.putText(img, char, tpos if char != "Space" else (tpos[0] - 20, tpos[1] - 10), cv.FONT_HERSHEY_PLAIN,
                       3 if char != "Space" else 1.5, (255, 255, 0), 2)
...
def hoverBtn(img, prop):
    ...
    cv.putText(img, char, tpos if char != "Space" else (tpos[0] - 20, tpos[1] - 10), cv.FONT_HERSHEY_PLAIN,
               3 if char != "Space" else 1.5, (255, 255, 255), 2)
